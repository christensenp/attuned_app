...
extension PitchEngine: SignalTrackerDelegate {
    ...
    public func signalTrackerDidFinished(audioFile: URL?) {
        self.delegate?.pitchEngineDidFinished(audioFile: audioFile)
    }
        ...
        queue.async { [weak self] in
            ...
            do {
                ...
                DispatchQueue.main.async { [weak self] in
                    guard let `self` = self else { return }
                    self.delegate?.pitchEngine(self, buffer: buffer, time: time, didReceivePitch: pitch)
                }
            } catch {
                DispatchQueue.main.async { [weak self] in
                    guard let `self` = self else { return }
