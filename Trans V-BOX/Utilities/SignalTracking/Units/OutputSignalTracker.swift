...
final class OutputSignalTracker: SignalTracker {
    ...
    func start() throws {
        ...
        audioPlayer.scheduleFile(audioFile, at: nil, completionHandler:  { [weak self] in
            guard let self = self else { return }
            self.delegate?.signalTrackerDidFinished(audioFile: nil)
        })

        audioPlayer.installTap(onBus: bus, bufferSize: bufferSize, format: nil) { buffer, time in
